...
export default function HostingPricingPlanCard({ headerTitle, items, footerDescription, headerDescription, price, pricingPeriod, variant = "secondary", animationDelay }: HostingPricingPlanCardProps) {
  return (
    <div
      ...
      <div className={twMerge("text-center px-5 py-6", variant === "primary" ? "bg-gradient-to-br from-blue-700 to-blue-500" : "bg-slate-800 ")}>
        ...
        <div className="mt-5">
          <ul className={twMerge("text-center divide-y", variant === "primary" ? "divide-white/20" : "divide-slate-700")}>
            {items.map((item) => (
              <li key={item.id} className={twMerge("text-sm lg:text-base py-3", variant === "primary" ? "text-white" : "text-slate-400")}>
                {item.name}
              </li>
            ))}
          </ul>
